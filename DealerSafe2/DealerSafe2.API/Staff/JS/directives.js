...
app.directive('editable', function () {
    return {
        ...
        replace: true,
        template: function (elm, attr) {
            return '<td><span ng-click="editField(enity, \'' + attr.editable + '\')" ng-if="!isFieldEdited(enity, \'' + attr.editable + '\')">{{entity.' + attr.editable + '}}</span><input ng-if="isFieldEdited(enity, \'' + attr.editable + '\')" type="text" ng-model="entity.' + attr.editable + '" ng-enter="saveField(enity, \'' + attr.editable + '\')"/></td>';
        }
    };
});
...
app.directive('infoRow', function () {
    return {
        ...
        replace: true,
        template: function (elm, attr) {
            var colSm = attr.colSm || 12;
            return '<div class="profile-info-row col-sm-'+colSm+'">' +
...
app.directive('inputLang', function () {
    return {
        ...
        replace: true,
        template: function (elm, attr) {
            return '<span ng-click="localize(\''+attr.fieldName+'\')" class="label label-success arrowed" style="cursor:pointer">'+
					'	{{localization.'+attr.fieldName+'.length || 0}} çeviri'+
					'</span>';
        }
    };
});

app.directive('inputId', function () {
    return {
        restrict: 'E',
        replace: true,
        template: function (elm, attr) {
            ...
            return '<div>' +
                    ...
                    '           <input type="text" maxlength="12" class="id-input col-sm-12 col-xs-12" ng-model="' + attr.model + '"' + (attr.name ? ' name="' + attr.name + '" id="' + attr.name + '"' : '') + (attr.disabled ? ' disabled="' + attr.disabled + '"' : '') + ' />' +
                    '           <span class="input-group-addon"><i class="icon-thumbs-down red"></i></span>' +
                        elm.html() +
                    '       </div>' +
                    '   </div>' +
                    '</div>';
        ...
        link: function (scope, element, attrs, controllers) {
            if (scope.entityId)
                element.find(".input-group-addon").html('<i class="icon-thumbs-up green"></i>');

            element.find(".id-input").blur(function () {
                var id = $(this).val();
                if (!id) {
                    element.find(".input-group-addon").html('<i class="icon-thumbs-down red"></i>');
                    return;
                }
                checkId(scope.entityName, id, function () {
                    element.find(".input-group-addon").html('<i class="icon-thumbs-down red"></i>');
                }, function () {
                    element.find(".input-group-addon").html('<i class="icon-thumbs-up green"></i>');
                });
    ...
    return {
        ...
        link: function (scope, element, attr) {
            ...
            function updateNavAndBreadCrumb() {
                ...
                //$(".sidebar .nav ul.submenu").css('display','none');

                $(".sidebar a[href='" + location.hash + "']").parent('li').addClass('active');
                var navParent = $('.sidebar .nav li.active').parent().closest('li');
                navParent.addClass("active").addClass('open');
                navParent.find('.sidebar .submenu').css('display', 'block');
